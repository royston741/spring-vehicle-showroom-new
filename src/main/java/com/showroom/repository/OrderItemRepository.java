...
//    Page<OrderItem> findOrderByVehicleName(@Param("vehicleName") String vehicleName,  Pageable page);
    ...
    @Query("Select i.vehicle.name, sum(i.quantity), sum(i.finalPrice) from OrderItem i  where i.order.orderDate between :startDate and :endDate group by i.vehicle.name")
