...
//    Page<OrderItem> findOrderByVehicleName(@Param("vehicleName") String vehicleName,  Pageable page);
    ...
    @Query("Select i.vehicle.name, count(i.vehicle.id), sum(i.finalPrice) from OrderItem i  where i.order.orderDate between :startDate and :endDate group by i.vehicle.id")
    public List<Object[]> getOrderItemDetails(@Param("startDate") LocalDate name, @Param("endDate") LocalDate phoneNo);
